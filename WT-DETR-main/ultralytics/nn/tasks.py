...
class RTDETRDetectionModel(DetectionModel):
    ...
    def __init__(self, cfg='rtdetr-l.yaml', ch=3, nc=None, verbose=True):
        ...
        """
        super().__init__(cfg=cfg, ch=ch, nc=nc, verbose=verbose)
        self.dowconv = nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1)
        self.conv = nn.Conv2d(512, 256, kernel_size=1)
        self.upsample = nn.Upsample(scale_factor=2, mode='nearest')
        self.conv1 = nn.Conv2d(in_channels=512,out_channels=768,kernel_size=1)

        self.hifa = nn.ModuleList()
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, Bottleneck, Focus,
                 C1, C2, C2f, C3, nn.Conv2d, nn.ConvTranspose2d, DWConvTranspose2d, RepC3,
                 ConvNormLayer):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
...
# ******************************************************************
        ...
        elif m in {transnext_micro, transnext_tiny, transnext_small, transnext_base}:
            m = m(*args)
            c2 = m.channel
        elif m in {SpatialAttention, CAA}:
            c2 = ch[f]
            args = [c2, *args]
